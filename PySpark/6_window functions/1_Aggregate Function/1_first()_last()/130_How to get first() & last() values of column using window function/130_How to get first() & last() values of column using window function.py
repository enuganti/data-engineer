...
# MAGIC %md
# MAGIC **return first value**
# MAGIC
# MAGIC      df.select(first("salary"))     
# MAGIC      df.select(first(df.salary, ignorenulls=True)) # To return the first non-null value instead
# MAGIC
# MAGIC **group by & aggregate**
# MAGIC
# MAGIC      df.groupBy("department") \
# MAGIC        .agg(first("salary").alias("First_Salary"), last("salary").alias("Last_Salary"))
# MAGIC
# MAGIC      df.groupBy("department") \
# MAGIC        .agg(first("salary", ignorenulls=True).alias("First_Salary"), last("salary", ignorenulls=True).alias("Last_Salary")) \
# MAGIC        .orderBy("department")
# MAGIC
# MAGIC **window function**
# MAGIC
# MAGIC      from pyspark.sql.window import Window
# MAGIC      df_window_null = df.withColumn("first_salary", first("salary").over(Window.partitionBy("department"))) \
# MAGIC                         .withColumn("last_salary", last("salary").over(Window.partitionBy("department"))) \
# MAGIC                         .orderBy("department")
# MAGIC
# MAGIC      df_window_ignore = df.withColumn("first_salary", first("salary", ignorenulls=True).over(Window.partitionBy("department"))) \
# MAGIC                           .withColumn("last_salary", last("salary", ignorenulls=True).over(Window.partitionBy("department"))) \
# MAGIC                           .orderBy("department")
